...
export default function LevelDetail() {
  ...
  if (!authLoading && !isAuthenticated) {
    return (
      <div className="max-w-md mx-auto text-center py-20">
        ...
        <p className="text-gray-600 mb-8">登录后即可开始闯关挑战</p>
        <button
          onClick={loginWithGitHub}
          className="flex items-center justify-center space-x-2 bg-gray-900 text-white px-6 py-3 rounded-lg font-medium hover:bg-gray-800 mx-auto"
        >
  ...
  useEffect(() => {
    if (level) {
      const existingProgress = progress.find(p => p.level_id === level.id);
  ...
  const goToNextLevel = () => {
    if (isLastInStage && level) {
      ...
      if (level.stage === 4) {
        ...
        router.push(`/stages/${level.stage}/complete`);
      }
    } else {
      const nextLevel = levels.find(l => l.id === level!.id + 1);
      if (nextLevel) {
        router.push(`/levels/${nextLevel.id}`);
      }
    }
  };
